...
TRE $c$, $d$ and $e$ where all chosen to benefit no pruning while generating, serving as a best case scenario for N, S and E.
...
We have shown that, in UPPAAL, you can gain significant performance by pruning the states, so pruning should be worth it to do most of the time, since pruning is a startup cost.

Pruning by itself can lead to really explosive performance on some regular expressions, even if we prune while the automaton is generating.
