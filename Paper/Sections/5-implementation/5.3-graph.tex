...
In the implementation, reversible edges are completely omitted. This is done because some reversible edges are self-looping which do not change when reversed $(ex. q9\rightarrow q9)$, and handling all reversible edges equally makes the algorithm much simpler. This could, however, introduce unforeseen edge crossings when reintroducing the reversible edges, but we chose the simpler algorithm. A reversible transition can be seen on \cref{fig:graph_step1} (dashed line), described as $q2\rightarrow q1$.
% mention self loops
