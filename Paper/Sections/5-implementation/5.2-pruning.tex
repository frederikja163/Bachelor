...
The amount of clocks in the automaton can be reduced after generating the TA.
...
Transitions that have unsatisfiable constraints, are considered "dead" transitions, and can be removed without changing the language of the automaton.
...
Since these states can never be reached, they are not part of the transition system for any words recognized by the automaton, hence it does not change the language to remove them.
...
Since these states can not reach final states, they are not part of the transition system for any words recognized by the automaton, hence it does not change the language to remove them.
...
Clocks that are never used in ranges to constrain any transitions are considered "dead", and can be removed without changing the language of the automaton.
...
This means we can remove the clock, since it does not effect the ability to take any transitions.
...
The recursion will take care of that and turn the automaton into the one in \cref{fig:prune6}.
