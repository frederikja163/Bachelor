...
In the automaton in \cref{fig:prune_0} both clock $c_1$ and $c_2$ can be combined into one clock since they are reset at the same time.
...
After $q7$ has been removed $q3$ is no longer the source of any transition.
This means $q3$ can also be removed.
This results in the simplest possible automata, one that has only one initial state and zero clocks.
